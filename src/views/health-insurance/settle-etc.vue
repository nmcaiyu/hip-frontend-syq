		...
		<section class="main-section flex-grow flex-column flex-align-stretch">
			...
			<div class="flex-column flex-align-center">
				<template v-if="step === 'AUTH'">
					<img :src="`${$publicPath}icons/health-insurance-card-colorful-etc.png`">
					<span>请扫电子医保码 <i class="el-icon-loading" /></span>
				</template>
...
export default {
	...
	methods: {
		...
		authenticate() {
			...
			if (!this.personInfo) return false;
			this.getBalance(this.personInfo.userName, this.personInfo.idNo).then((balance) => {
				this.infoList = [
					{ label: '姓名', value: this.personInfo.userName },
					{ label: '证件号码', value: this.personInfo.idNo },
					{
						label: '出生日期',
						value: `${this.personInfo.idNo.substr(6, 4)}/${this.personInfo.idNo.substr(
							10,
							2
						)}/${this.personInfo.idNo.substr(12, 2)}`,
					},
					{
						label: '账户余额',
						value: balance,
		...
		preSettle() {
			...
			const postData = {
				...
				medicareCardInfo: this.personInfo.idNo,
			};
